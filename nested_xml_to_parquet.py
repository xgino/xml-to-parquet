...
class MultiNestedXMLToParquet:
    ...
    def parse_xml_to_dict(self, xml_file_path):
        ...
        for event, element in tqdm(context, desc="Processing XML", unit="element"):
            ...
            if tag == 'measurementSiteReference' and 'id' in element.attrib:
                record = {'measurementSiteReference_id': element.attrib['id']}
                logger.debug(f"Found measurementSiteReference: {record}")

            elif tag == 'measuredValue':
                ...
                for key, value in flattened.items():
                    ...
                    if key in record:
                        ...
                        record[key] = value
                self.logger.debug(f"Parsed measuredValue: {record}")
                yield record
...
if __name__ == "__main__":
    xml_file_path = "traveltime.xml"
    output_file = "traveltime.parquet"
